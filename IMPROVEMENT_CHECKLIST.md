...
- [x] Stop duplicate network fetches in the client sync flow.
- [x] Reduce initial page load weight by trimming unnecessary requests and renders.
- [x] Confirm the app does not fetch the same data more than once during startup.
- [x] Make sure cron jobs are updating standings on schedule.
- [x] Verify the leaderboard refresh path runs after score updates and standings updates.
- [x] Keep rank numbers visible and readable on small screens.
- [x] Reduce overly large card spacing in prediction views.
- [x] Use sharp flag images on retina screens.
- [x] Keep layout and image aspect ratios stable so flags do not appear stretched or flipped.
- [x] Update any doc that still mentions stale scoring or data-source behavior.
- [x] Keep setup notes, scoring notes, and troubleshooting notes synchronized.
...
- [x] Keep polling intervals reasonable for the amount of live data that actually changes.
- [x] Make long player names fit without excessive truncation.
- [ ] Keep team flag code mapping in sync with fixture team names.
- [x] Verify Bosnia and Herzegovina, USA, and DR Congo flag mappings still resolve correctly.
...
- [x] Record major fixes in a short changelog or worklog entry.
...
- [x] Avoid recomputing the leaderboard on every read if a cached table already exists.
...
- [x] Keep leaderboard rows compact on phones.
- [x] Prevent the leaderboard from sliding horizontally on mobile.
...
- [x] Keep markdown docs aligned with runtime behavior.
