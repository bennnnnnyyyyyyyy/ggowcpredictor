---
title: "Fix Plan"
description: "Prioritized repair plan — scoring, leaderboard, bracket, admin tooling, and Supabase migration."
---

## Summary

Prioritize a canonical scoring repair, then rebuild leaderboard data from Supabase rows, fix
bracket prefill/seeding, remove Firestore paths, clean up UI/CSS, and add admin tooling.

Base scoring: `15 / 8 / 5 / 0`. Knockout multipliers: R32 `2x`, R16 `2.5x`, QF `3x`, SF/third `4x`,
Final `5x`. Full detail on [Scoring](/scoring).
...
- `normalizeWorldcup26Games` maps `home_penalty_score`/`away_penalty_score` → `homePenalty`/`awayPenalty`
...
<AccordionGroup>
  <Accordion title="Admin panel — remaining features">
  ...
  <Accordion title="Bracket">
    ...
    - Confirm bracket auto-advances on next `renderBracket()` call after a `penalty_winner` DB patch
  </Accordion>
  <Accordion title="Supabase migration">
    - Remove remaining Firestore read/write calls from `app.js` and the worker
...
- API `id` field ≠ internal `matchId` for some knockout games — always join through `apiFixtureId`, never assume equality
