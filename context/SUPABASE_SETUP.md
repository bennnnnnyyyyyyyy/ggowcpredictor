...
## Homepage support SQL

Run this in the Supabase SQL Editor if you want database-side support for the homepage inbox and admin filtering:

```sql
alter table if exists accountRequests
  add column if not exists is_admin boolean not null default false;

create index if not exists accountRequests_status_idx
  on accountRequests (status);

create index if not exists accountRequests_is_admin_idx
  on accountRequests (is_admin);

create index if not exists predictions_username_match_idx
  on predictions (username, "matchId");

create index if not exists predictions_match_idx
  on predictions ("matchId");

create or replace view home_suggestions as
select
  username,
  "displayName",
  note,
...
The frontend still filters admin requests on the client, but this view makes the rule explicit in Supabase too and keeps the inbox query easy to reuse later.
