...
Starts when group stage ends. Only knockout rounds count in the Mini Tourney bonus scoring.
...
| Round of 32 | ×2 |
| Round of 16 | ×2.5 |
| Round of 8 / Quarter-final | ×3 |
| Round of 4 / Semi-final | ×4 |
| Final / 3rd Place | ×5 |
...
- `src/main.js:scoreMatch` — **UPDATE** to reflect the final 15/8/5/0 rule set and knockout stage multipliers.
- `scripts/app.js:calculateMatchPoints` — **UPDATE** to match exactly.
- Multipliers should be derived from the fixture's `stage` field (`group`, `r32`, `r16`, `qf`, `sf`, `third`, `final`).
