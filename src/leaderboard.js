  ...
  try {
    ...
    function fetchDocs(collection) {
      var resp = UrlFetchApp.fetch(
        base + "/" + collection + "?pageSize=500&key=" + apiKey,
    ...
    function fv(fields, k) {
      ...
      return fields[k].stringValue !== undefined
        ? fields[k].stringValue
        : fields[k].integerValue !== undefined
          ? fields[k].integerValue
          : fields[k].doubleValue !== undefined
    ...
    var leaderboardPayload = {
      fields: {
        players: {
          arrayValue: {
            values: leaderboard.map(function (p) {
              return {
                mapValue: {
                  fields: {
                    ...
                    correctOutcomes: {
                      integerValue: String(p.correctOutcomes),
                    },
    ...
    UrlFetchApp.fetch(leaderboardUrl, {
      ...
      payload: JSON.stringify(leaderboardPayload),
      muteHttpExceptions: true,
    });
    ...
    leaderboard.forEach(function (p) {
      var userUrl =
        ...
        "/users/" +
        p.username +
        "?updateMask.fieldPaths=rank&updateMask.fieldPaths=totalPoints&key=" +
        apiKey;
