...
let _reportScrollTrackerAttached = false;
function setupReportScrollSeenTracking() {
  if (_reportScrollTrackerAttached) return;
  _reportScrollTrackerAttached = true;

  const seenKey = SESSION.username
    ? `ggo_report_welcome_seen_${SESSION.username}`
    : "ggo_report_welcome_seen_guest";
...
function isTeamEliminated(teamName) {
  ...
  for (const f of fixtures) {
    const result = STATE.results?.[f.matchId];
    const isFinished =
      result && finishedStatuses.has(String(result.status || "").toUpperCase());
...
function isExcludedMatch(fixture) {
  ...
  return EXCLUDED_MATCH_TEAMS.some(
    ([a, b]) =>
...
function getTournamentChampion() {
  ...
  if (!FINISHED_STATUSES.has(String(result.status || "").toUpperCase()))
    return null;
  ...
  if (result.score1 > result.score2)
    return resolveSlot(final.team1) || final.team1;
  if (result.score2 > result.score1)
    return resolveSlot(final.team2) || final.team2;
  return null;
}
...
function computeTournamentReport() {
  ...
  for (const pred of allPredsList) {
    ...
    if (!player.perStage[sk])
      player.perStage[sk] = { pts: 0, exact: 0, correct: 0, games: 0 };
  ...
  for (const fixture of finishedFixtures) {
    ...
    const result = STATE.results[matchId];
    const preds = allPredsList.filter(
      (p) => String(p.matchId || p.match_id) === matchId && p.pred1 != null,
    );
    const pickCounts = {};
    const exactUsers = [];
    for (const p of preds) {
      ...
      const mult = STAGE_MULTIPLIERS[stage] || 1;
      const isFOT = stage === "final" || stage === "third";
      if (isFOT ? pts === 100 : Math.abs(pts / mult - 15) < 0.01)
        exactUsers.push(p.username);
    }
    ...
    matchStats[matchId] = {
      ...
      mostPopularPick: top?.[0] || null,
      mostPopularCount: top?.[1] || 0,
...
function computeAwards(players, matchStats, fixtures) {
  ...
  if (players[0])
    awards.push(
      A({
        icon: "💥",
        name: "GGO Champion",
        desc: "Highest total points — king of the table",
        winner: players[0].displayName,
        stat: `${players[0].totalPoints} pts`,
  ...
  if (oracle)
    awards.push(
      A({
        icon: "🎯",
        name: "The Oracle",
        desc: "Most exact scorelines across all 104 games",
        winner: oracle.displayName,
        stat: `${exactOf(oracle)} exact scores`,
  ...
  const koKey = (p) =>
    ["r32", "r16", "qf", "sf", "third", "final"].reduce(
  ...
  if (koKing)
    awards.push(
      A({
        icon: "🔥",
        name: "Knockout King",
        desc: "Most points earned in knockout rounds",
        winner: koKing.displayName,
        stat: `${koKey(koKing)} knockout pts`,
        username: koKing.username,
      }),
    );
  ...
  const sniper = srt(snipers, (a, b) => {
    const as = a.scored || a.perGame.length,
      bs = b.scored || b.perGame.length;
  ...
  if (sniper) {
    ...
    awards.push(
      A({
        icon: "🔭",
        name: "Sniper",
        desc: "Highest exact score rate (min 15 predictions)",
        winner: sniper.displayName,
        stat: `${Math.round((exactOf(sniper) / (s || 1)) * 100)}% exact rate`,
  ...
  for (const ms of Object.values(matchStats)) {
    if (!ms.excluded && ms.exactUsers.length === 1)
      uniqueExacts[ms.exactUsers[0]] =
        (uniqueExacts[ms.exactUsers[0]] || 0) + 1;
  }
    ...
    if (p)
      awards.push(
        A({
          icon: "🎲",
          name: "Lone Wolf",
          desc: "Most exact scores that nobody else predicted",
          winner: p.displayName,
          stat: `${lone[1]} solo exact scores`,
          username: p.username,
        }),
      );
  }
  ...
  if (withVar[0])
    awards.push(
      A({
        icon: "📐",
        name: "Mr. Consistent",
        desc: "Smallest variance in points game to game",
        winner: withVar[0].player.displayName,
        stat: `σ = ${Math.round(withVar[0].sd)} pts`,
        username: withVar[0].player.username,
      }),
    );
  ...
  if (finalFix && matchStats[finalFix.matchId]?.exactUsers?.length > 0) {
    const names = matchStats[finalFix.matchId].exactUsers
      .map((u) => players.find((x) => x.username === u)?.displayName || u)
      .join(", ");
    awards.push(
      A({
        icon: "🏆",
        name: "Final Boss",
        desc: "Predicted the World Cup Final scoreline exactly",
        winner: names,
        stat: "+100 pts",
        username: matchStats[finalFix.matchId].exactUsers[0],
      }),
    );
  }
  ...
  const hb = srt(
    players.filter((p) => exactOf(p) === 0),
    (a, b) => (b.correctOutcomes || 0) - (a.correctOutcomes || 0),
  )[0];
  if (hb)
    awards.push(
      Q({
        icon: "😬",
        name: "Heartbreak Kid",
        desc: "Knew the winner, never nailed the exact score",
        winner: hb.displayName,
        stat: `${hb.correctOutcomes || 0} correct, 0 exact`,
  ...
  const cold = srt(
    players.filter((p) => (p.scored || p.perGame.length) >= 15),
    (a, b) => {
      const as = a.scored || a.perGame.length,
        bs = b.scored || b.perGame.length;
      return (
        (a.correctOutcomes || 0) / (as || 1) -
        (b.correctOutcomes || 0) / (bs || 1)
      );
    },
  )[0];
  if (cold) {
    ...
    awards.push(
      Q({
        icon: "🧊",
        name: "Ice Cold",
        desc: "Lowest correct outcome rate — rough tournament",
        winner: cold.displayName,
        stat: `${Math.round(((cold.correctOutcomes || 0) / (s || 1)) * 100)}% correct`,
  ...
  const optim = srt(
    players.map((p) => ({
      p,
      total: p.perGame.reduce(
        (s, g) => s + (Number(g.pred1) || 0) + (Number(g.pred2) || 0),
        0,
      ),
    })),
    (a, b) => b.total - a.total,
  ...
  if (optim)
    awards.push(
      Q({
        icon: "📉",
        name: "The Optimist",
        desc: "Predicted the most total goals across all games",
  ...
  if (players.length > 1) {
    ...
    awards.push(
      Q({
        icon: "💀",
        name: "Last Place",
        desc: "Better luck at the next World Cup",
        winner: last.displayName,
        stat: `${last.totalPoints} pts`,
...
function computeHighlights(players, matchStats) {
  ...
  if (mostExact?.exactCount > 0) {
    ...
    h.push({
      icon: "🎯",
      label: "Most people nailed it",
      text: `${mostExact.exactCount} player${mostExact.exactCount > 1 ? "s" : ""} called ${resolveSlot(f.team1) || f.team1} ${mostExact.result.score1}–${mostExact.result.score2} ${resolveSlot(f.team2) || f.team2} exactly`,
    });
  }

  // Match where everyone who predicted scored 0
  const zeroGame = validMs.find(
    (ms) =>
      ms.preds.length >= 3 &&
      ms.preds.every((p) => {
        const pts = calculateMatchPoints(
  ...
  if (zeroGame) {
    ...
    h.push({
      icon: "💀",
      label: "Nobody scored on this one",
      text: `Not a single player got points on ${resolveSlot(f.team1) || f.team1} ${zeroGame.result.score1}–${zeroGame.result.score2} ${resolveSlot(f.team2) || f.team2}`,
    });
  }

  // Biggest upset — winning team was least predicted
  const upsets = validMs
    .map((ms) => {
      const r = ms.result;
      ...
      const winnerPicks = ms.preds.filter((p) =>
        winner === "home" ? p.pred1 > p.pred2 : p.pred2 > p.pred1,
      ).length;
  ...
  if (upsets[0] && upsets[0].pct <= 30) {
    const f = upsets[0].ms.fixture;
    const r = upsets[0].ms.result;
    const winner =
      r.score1 > r.score2
        ? resolveSlot(f.team1) || f.team1
        : resolveSlot(f.team2) || f.team2;
    h.push({
      icon: "😱",
      label: "Biggest upset",
      text: `Only ${upsets[0].pct}% predicted ${winner} to win — nobody saw it coming`,
    });
  }

  // Game most people got correct (not just exact)
  const mostCorrect = [...validMs]
    .filter((ms) => ms.preds.length >= 3)
    .sort((a, b) => {
      const ac = a.preds.filter(
        (p) =>
          calculateMatchPoints(
            p.pred1,
            p.pred2,
            ...
            a.fixture.stage,
            p.pen_winner,
            a.result.penalty_winner,
          ) > 0,
      ).length;
      const bc = b.preds.filter(
        (p) =>
          calculateMatchPoints(
            p.pred1,
            p.pred2,
  ...
  if (mostCorrect) {
    ...
    const cnt = mostCorrect.preds.filter(
      (p) =>
        calculateMatchPoints(
          p.pred1,
          p.pred2,
          mostCorrect.result.score1,
          mostCorrect.result.score2,
          f.stage,
          p.pen_winner,
          mostCorrect.result.penalty_winner,
        ) > 0,
    ).length;
    const pct = Math.round((cnt / mostCorrect.preds.length) * 100);
    h.push({
      icon: "🤝",
      label: "Everyone agreed on this one",
      text: `${pct}% of players called ${resolveSlot(f.team1) || f.team1} vs ${resolveSlot(f.team2) || f.team2} correctly`,
    });
  }

  // Most predicted game (most engaged match)
  const mostPred = [...validMs].sort(
  ...
  if (mostPred) {
    ...
    h.push({
      icon: "🏟️",
      label: "Most predicted game",
      text: `${mostPred.totalPredicted} players predicted ${resolveSlot(f.team1) || f.team1} vs ${resolveSlot(f.team2) || f.team2}`,
    });
  }

  // Least predicted game among properly live ones (min 1 prediction, well below average)
  const avgPred =
    validMs.reduce((s, m) => s + m.totalPredicted, 0) / (validMs.length || 1);
  const leastPred = [...validMs]
    .filter((ms) => ms.totalPredicted > 0 && ms.totalPredicted < avgPred * 0.6)
    .sort((a, b) => a.totalPredicted - b.totalPredicted)[0];
  if (leastPred) {
    const f = leastPred.fixture;
    h.push({
      icon: "🥱",
      label: "Least predicted game",
      text: `Only ${leastPred.totalPredicted} players predicted ${resolveSlot(f.team1) || f.team1} vs ${resolveSlot(f.team2) || f.team2}`,
    });
  }
...
function renderTournamentReport() {
  ...
  const champion = getTournamentChampion();
  const { players, matchStats, finishedFixtures, awards, highlights } =
    computeTournamentReport();
  ...
  const activeStageKeys = STAGE_KEYS.filter((sk) =>
    players.some((p) => (p.perStage[sk]?.pts || 0) > 0),
  );
  ...
  const scorecardsHtml = players.length
    ...
    <section class="tr-section">
      ...
      .map((p) => {
        ...
        const pct =
          sc > 0 ? Math.round(((p.correctOutcomes || 0) / sc) * 100) : 0;
        const best = p.perGame.length
          ? [...p.perGame].sort((a, b) => b.pts - a.pts)[0]
          : null;
        const worstMiss = p.perGame
          .filter((g) => g.pts === 0)
          .sort(() => 0)[0];
            ...
            <div class="tr-scorecard${isMe ? " tr-scorecard-me" : ""}" id="sc-${CSS.escape ? CSS.escape(p.username) : p.username}">
              ...
              <div class="tr-sc-body">
                ...
                <div class="tr-sc-stages">
                  ${STAGE_KEYS.filter((sk) => p.perStage[sk])
            .map((sk) => {
  ...
  const gamesHtml = finishedFixtures.length
    ...
    <section class="tr-section">
      ...
      .reverse()
      .map((f) => {
        const ms = matchStats[f.matchId];
        if (!ms) return "";
        const r = ms.result;
        const t1 = resolveSlot(f.team1) || f.team1,
          t2 = resolveSlot(f.team2) || f.team2;
        const stage = String(f.stage || "group").toLowerCase();
        const stageLbl = STAGE_LABELS[stage] || stage;
        const correctCount = ms.preds.filter(
          (p) =>
            calculateMatchPoints(
              p.pred1,
              p.pred2,
